"use client";

import dynamic from "next/dynamic";
import { motion, useReducedMotion } from "motion/react";
import { CONTACT_EMAIL } from "@/lib/contact";
import { DiscoveryCallForm } from "@/components/v2/DiscoveryCallForm";
import { LeaveMessageForm } from "@/components/v2/LeaveMessageForm";

const LiquidChrome = dynamic(() => import("@/components/ui/LiquidChrome"), {
  ssr: false,
});

const EASE_OUT_STRONG = [0.23, 1, 0.32, 1] as const;
const LIQUID_BASE_COLOR: [number, number, number] = [0.05, 0.06, 0.12];

export function V2Contact() {
  const reduceMotion = useReducedMotion();

  return (
    <section id="contact" className="py-32">
      <div
        className="relative mx-auto max-w-6xl overflow-hidden rounded-xl border px-6 py-16"
        style={{ borderColor: "var(--v2-border)" }}
      >
        {!reduceMotion && (
          <div className="absolute inset-0 z-0" aria-hidden="true">
            <LiquidChrome
              baseColor={LIQUID_BASE_COLOR}
              speed={0.35}
              amplitude={0.35}
              frequencyX={2.5}
              frequencyY={1.8}
              interactive
            />
          </div>
        )}

        <div className="relative z-10">
          <div className="pointer-events-none text-center">
            <motion.h2
              className="v2-display mx-auto max-w-4xl text-[clamp(2.5rem,6vw,4.25rem)]"
              initial={reduceMotion ? false : { opacity: 0, y: 24 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, amount: 0.4 }}
              transition={{ duration: 0.7, ease: EASE_OUT_STRONG }}
            >
              Let&apos;s build something.
            </motion.h2>

            <motion.p
              className="mx-auto mt-6 max-w-md text-lg leading-relaxed"
              style={{ color: "var(--v2-muted)" }}
              initial={reduceMotion ? false : { opacity: 0, y: 16 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, amount: 0.4 }}
              transition={{ duration: 0.6, delay: 0.1, ease: EASE_OUT_STRONG }}
            >
              Leave a message or request a discovery call — whichever is easier.
            </motion.p>
          </div>

          <motion.div
            className="pointer-events-auto mt-12 grid gap-6 lg:grid-cols-2 lg:gap-8"
            initial={reduceMotion ? false : { opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.2 }}
            transition={{ duration: 0.6, delay: 0.15, ease: EASE_OUT_STRONG }}
          >
            <div
              className="relative rounded-xl border px-5 py-6 sm:px-6"
              style={{
                borderColor: "var(--v2-border)",
                background: "rgba(14,15,18,0.78)",
              }}
            >
              <LeaveMessageForm />
            </div>
            <div
              className="relative rounded-xl border px-5 py-6 sm:px-6"
              style={{
                borderColor: "var(--v2-border)",
                background: "rgba(14,15,18,0.78)",
              }}
            >
              <DiscoveryCallForm />
            </div>
          </motion.div>

          <div
            className="pointer-events-auto mx-auto mt-12 flex flex-col items-center gap-3 text-sm sm:flex-row sm:justify-center sm:gap-10"
            style={{ color: "var(--v2-muted)" }}
          >
            <a
              href={`mailto:${CONTACT_EMAIL}`}
              className="v2-mono transition-colors hover:text-[var(--v2-text)]"
            >
              {CONTACT_EMAIL}
            </a>
            <a
              href="https://www.linkedin.com/in/clyde-keanu-abenojar-b3b578346"
              target="_blank"
              rel="noopener noreferrer"
              className="v2-mono transition-colors hover:text-[var(--v2-text)]"
            >
              LinkedIn
            </a>
            <a
              href="https://github.com/clydekeanu-rgb"
              target="_blank"
              rel="noopener noreferrer"
              className="v2-mono transition-colors hover:text-[var(--v2-text)]"
            >
              GitHub
            </a>
          </div>
        </div>
      </div>
    </section>
  );
}
